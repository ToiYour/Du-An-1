...
foreach ($list_comment as $value) : $i++;
    ...
    <div class="wapper-comment">
        <div class="d-flex position-relative commnet">
            <img src="assets/images/avatar/<?= $value['hinh_anh'] ?>" alt="" style="width: 40px; height: 40px; border-radius: 50%;">
            <div class="info-comment ms-2" style="width: 100%;">
                <h6 class="m-0"><?= $value['ho_ten'] ?></h6>
                <p class="noiDung mb-0"><?= $value['noi_dung'] ?></p>
                <textarea class="form-control ms-2 border-bottom" placeholder="" id="floatingTextarea" style="display: none; width: 100%; overflow: hidden; resize: none; border: none; outline: none; box-shadow: none;"><?= $value['noi_dung'] ?></textarea>
                ...
                ?>
                    <p class="d-flex gap-1 mb-1 dropdown-comment">
                        <a class="text-muted btn-link" style="font-size: 12px;" data-bs-toggle="collapse" href="#collapseExample<?= $i ?>" role="button" aria-expanded="false" aria-controls="collapseExample<?= $i ?>">
                            Xem thêm câu trả lời phía cửa hàng<i class="fa fa-angle-down ms-1" aria-hidden="true"></i>
                        </a>
                    </p>
                <?php endif ?>
                <button type="button" class="btn-edit-comment btn btn-info px-2 py-0 float-end mt-1 fw-normal " style="display: none; border:none; color: #333;">Lưu</button>
                <div class="feature-commnet position-absolute top-0 end-0 dropdown">

                    <i class="fa fa-ellipsis-v px-3 pt-3 pb-1 btn-feature-comment" aria-hidden="true" data-bs-toggle="dropdown" aria-expanded="false" style="cursor: pointer;"></i>
                    <ul class="dropdown-menu dropdown-menu-end" style="margin-top: -5px;">
                        <?php if (isset($_SESSION['login']) && $_SESSION['login']['id_kh'] == $value['id_kh']) : ?>
                            <li><a class="dropdown-item edit-comment" href="<?= $value['id_binh_luan'] ?>"><i class="fas fa-pen pe-3"></i>Chỉnhsửa</a>
                            </li>
                            <li><a class="dropdown-item delete-comment" href="<?= $value['id_binh_luan'] ?>"><i class="far fa-trash-alt pe-3"></i>Xoá</a></li>
                        <?php else : ?>
                            <li><a class="dropdown-item repost-comment" href="<?= $value['id_binh_luan'] ?>"><i class="far fa-flag pe-3"></i>Báo vi
                                    phạm</a></li>
                        <?php endif ?>
                    </ul>
                </div>
            ...
            <div class="collapse ms-5" id="collapseExample<?= $i ?>">
                <div class=" commnet-feedback">
                    <div class="d-flex position-relative">
                        <img src="assets/images/avatar/<?= $value['hinh_anh'] ?>" alt="" style="width: 24px; height: 24px; border-radius: 50%;">
